...
## Garnet 
Garnet is the first Fuchsia layer above the kernel. It contains a variety of low-level things that every OS needs including device drivers (network, graphics, etc) and software installation.

The Garmet UI services are called `Mozart`.
...
[Simple Example](https://fuchsia.googlesource.com/peridot/+/refs/heads/master/examples/simple/)
...
### Fuchsia Core SDK
The Core SDK contains a small set of libraries and tools required to start building and running programs that target Fuchsia
...
- [Developing a Package](workflows/package_update.md)
